...
## 11. Cách viết nhận xét kết quả trong báo cáo

Có thể diễn giải ngắn như sau:

> Kết quả thực nghiệm cho thấy mô hình BAMA và E-BAMA trên GTSRB với backbone MobileNetV2 ở chế độ feature extraction có độ nhạy cao với các tấn công gradient-based. Cụ thể, PGD và MI-FGSM đạt tỷ lệ tấn công thành công 100%, trong khi FGSM và CW cũng đạt tỷ lệ rất cao. Ngược lại, CAN gần như không hiệu quả trong cấu hình hiện tại. Điều này cho thấy khả năng chống chịu của mô hình phụ thuộc mạnh vào loại attack, ngân sách nhiễu và cách huấn luyện mô hình.

Nếu viết kỹ hơn:

> Việc PMA có tỷ lệ tấn công thành công thấp hơn BAMA/E-BAMA trong cùng nhóm kết quả cho thấy sự khác biệt rõ rệt giữa các baseline. Tuy nhiên, kết quả này cần được đọc trong phạm vi cấu hình hiện tại, bao gồm dataset, backbone, số ảnh đánh giá và tham số attack. Để kết luận chắc chắn hơn, cần mở rộng số lượng ảnh, chạy nhiều seed và so sánh thêm trên các backbone khác như InceptionV3 và ResNet50V2.

---

## 12. Lưu ý và giới hạn hiện tại

Một số điểm cần chú ý khi đọc repo:

1. File `appendix_outputs/attack_summary.csv` là nguồn kết quả nên đọc trước tiên.
2. File `attack_summary.csv` ở thư mục gốc có ít dòng hơn và có vẻ là bản chạy nhanh/trung gian.
3. Một số đường dẫn trong summary trỏ tới `results_json/`, nhưng thư mục này có thể chưa được commit đầy đủ.
4. Kết quả trong summary chủ yếu thể hiện rõ cho nhóm `GTSRB + MobileNetV2`; nếu muốn kết luận rộng hơn cho CIFAR10, FLOWERS, InceptionV3 hoặc ResNet50V2 thì cần kiểm tra thêm log/JSON tương ứng.
5. `num_images = 50` là quy mô đánh giá còn nhỏ; kết quả phù hợp để minh họa thực nghiệm, nhưng nếu viết bài nghiên cứu nghiêm túc thì nên tăng số ảnh và chạy nhiều seed.
6. Các attack khác nhau dùng cơ chế khác nhau, nên không nên so sánh chỉ bằng ASR mà bỏ qua epsilon, số bước, norm và cách sinh ảnh đối kháng.
7. ASR cao không có nghĩa attack “tốt nhất tuyệt đối”; nó chỉ có nghĩa attack hiệu quả trong cấu hình đang xét.

---

## 13. Kết luận nhanh

Repo này cho thấy pipeline thực nghiệm tương đối đầy đủ: tạo dữ liệu, huấn luyện mô hình, chuyển TFLite, chạy attack, lưu adversarial examples và tổng hợp kết quả. Với kết quả hiện tại, các attack dựa trên gradient như FGSM, CW, PGD và MI-FGSM gây ảnh hưởng mạnh tới BAMA/E-BAMA trên GTSRB + MobileNetV2, trong khi CAN gần như không hiệu quả.

Khi trình bày repo này, nên nhấn mạnh ba ý:

1. Đây là thực nghiệm đánh giá độ nhạy của mô hình trước ảnh đối kháng.
2. Chỉ số quan trọng nhất là `mean_success_rate` trong `appendix_outputs/attack_summary.csv`.
3. Kết quả hiện tại cho thấy BAMA/E-BAMA dễ bị tấn công trong nhiều cấu hình, đặc biệt với PGD và MI-FGSM.
